...
class ConnegCheck(SubRequest):
    ...
    def modify_req_hdrs(self, req_hdrs):
        return [h for h in req_hdrs 
                  if h[0].lower() != 'accept-encoding'] + \
               [(u'accept-encoding', u'gzip')]

    def preflight(self):
        ae = ",".join([h[1] for h in self.base.request.headers if h[0].lower() == 'accept-encoding'])
        aev = [v.strip() for v in ae.split(',')]
        if 'gzip' in aev or 'x-gzip' in aev:
            return False
        return True
    ...
    def done(self):
        ...
        no_conneg_vary_headers = \
          self.base.response.parsed_headers.get('vary', [])
        if 'gzip' in self.base.response.parsed_headers.get('content-encoding', []) \
          or 'x-gzip' in self.base.response.parsed_headers.get('content-encoding', []):
            ...
            if self.base.response.status_code != \
               self.response.status_code:
                self.add_note('status', rs.VARY_STATUS_MISMATCH, 
                  neg_status=self.response.status_code,
                  noneg_status=self.base.response.status_code)
            ...
            # check Vary headers
            vary_headers = self.response.parsed_headers.get('vary', [])
            ...
            if (self.response.parsed_headers.get('etag', 1) == \
              self.base.response.parsed_headers.get('etag', 2)):
                if not self.response.parsed_headers['etag'][0]: # strong
                    self.add_note('header-etag',
                        rs.VARY_ETAG_DOESNT_CHANGE
                    ) 

            # check compression efficiency
            if self.base.response.payload_len > 0:
                savings = int(100 * 
                    (
                        (float(self.base.response.payload_len) - \
                ...
                savings = 0
            self.test_state.gzip_support = True
            self.test_state.gzip_savings = savings
            if savings >= 0:
                self.add_note('header-content-encoding',
                    rs.CONNEG_GZIP_GOOD,
                    savings=savings,
                    orig_size=f_num(self.base.response.payload_len),
                    gzip_size=f_num(self.response.payload_len)
                )
            else:
                self.add_note('header-content-encoding',
                    rs.CONNEG_GZIP_BAD,
