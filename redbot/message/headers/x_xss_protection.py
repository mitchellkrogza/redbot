...
def parse(subject, value, red):
    ...
    if protect == 0:
        red.exchange_state.add_note(subject, rs.XSS_PROTECTION_OFF)
    else: # 1
        if params.get('mode', None) == "block":
            red.exchange_state.add_note(subject, rs.XSS_PROTECTION_BLOCK)
        else:
            red.exchange_state.add_note(subject, rs.XSS_PROTECTION_ON)
